<?php
// 1. Initialize session and security guard
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

// Ensure user is authorized as a renter
...
// 3. Validate the incoming Property ID from the URL
...
// 4. Fetch specific property details, its lowest rent, and its primary image
$property = null;
$query = "SELECT 
            p.property_id, 
            p.property_name, 
...
// If no matching property is found in the database
if (!$property) {
    die("Error: The requested property could not be found or is no longer available.");
}

// 5. Query to fetch the specific accommodations list
// 🔍 DEBUG REMOVAL: Removed 'AND status = available' to see if rows pull at all!
$accommodationsList = [];
        ...
        <main class="main">
         ...
         <h2>Property Specifications</h2>
            ...
            <section class="booking-action-panel">
                <button class="primary-action-btn" onclick="alert('Proceeding to reservation or inquiry process...');">Inquire About This Property</button>
                <button class="secondary-action-btn" onclick="window.location.href='messages.php?recipient_id=<?php echo $ownerId; ?>&property_id=<?php echo $propertyId; ?>';">Message Owner</button>
            </section>
        </main>
    </div>
</body>
